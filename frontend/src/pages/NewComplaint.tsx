...
export default function NewComplaint() {
  ...
  const locate = () => {
    ...
    if (!navigator.geolocation) {
      setGeoError("Location is not supported here. Tap the map to place the pin.");
      return;
    }
    setGeoLocating(true);
    navigator.geolocation.getCurrentPosition(
      ...
      (geo) => {
        setGeoLocating(false);
        setGeoError(`${geo.code === geo.PERMISSION_DENIED ? "Location permission is off." : "We could not find your location."} ${"Tap the map to place the pin, or try again."}`);
      },
  ...
  if (result) {
    return <section className="filed-receipt" aria-labelledby="filed-title">
      ...
      <span className="filed-stamp">{"Complaint registered"}</span>
      ...
      {result.status === "duplicate" && result.report_count !== null && <p className="filed-receipt__grouped">{fill("{count} citizens have reported this issue. Your report has been grouped with theirs.", { count: result.report_count })}</p>}
  ...
  return <div className="wizard-page">
    <header className="wizard-header">
      ...
      <div><span>{fill("Step {current} of {total}", { current: step + 1, total: 4 })}</span><h1>{steps[step].title}</h1></div>
    </header>
    ...
    <div className="wizard-stage" key={step}>
      {step === 0 && <div className="wizard-split"><div><LocationPicker value={location} onChange={setLocation} recenter={recenter} /><label className="wizard-landmark"><span>{"Nearby landmark (optional)"}</span><input type="text" value={landmark} maxLength={160} placeholder={"e.g., opposite the water tank, near the primary school"} onChange={(event) => setLandmark(event.target.value)} /></label></div><div className="wizard-aside"><button type="button" className="wizard-locate" onClick={locate} disabled={geoLocating}><span className="wizard-locate__icon" aria-hidden="true">⌖</span>{geoLocating ? "Finding location…" : "Use my location"}</button>{geoError && <div className="notice" data-tone="warning" role="alert"><p>{geoError}</p></div>}<aside className="wizard-tip"><TipArtifact kind="location" /><b>{"Pin the exact place"}</b><p>{"Use your current location, tap the map, or drag the pin. A precise location helps the right team find the issue without calling you for directions."}</p><small>{"Location is attached only to this complaint."}</small></aside></div></div>}
      {step === 1 && <div className="wizard-split"><div className="wizard-description"><label className="visually-hidden" htmlFor="complaint-description">{"Describe the civic issue"}</label><textarea id="complaint-description" rows={7} placeholder={"What happened? Include a nearby landmark, how long the problem has existed, and any safety risk."} value={description} onChange={(event) => setDescription(event.target.value)} maxLength={2000} aria-describedby="description-count" /><span id="description-count" className="character-count">{fill("{count}/2000 characters · or add a voice note", { count: description.length })}</span><VoiceRecorder clips={clips} onClipsChange={setClips} previewEnabled /></div><aside className="wizard-tip"><TipArtifact kind="voice" /><b>{"Tell it in your own words"}</b><p>{"Type or record a voice note. Mention what is broken, when you noticed it, and whether anyone is at immediate risk."}</p><small>{"You need either text or one voice note."}</small></aside></div>}

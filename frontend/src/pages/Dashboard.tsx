...
export default function Dashboard() {
  ...
  return <div className="app-page dashboard-page">
    ...
    {query.data && query.data.length > 0 && <div className="complaint-table" role="table" aria-label={"Your complaints"}><div className="complaint-table__head" role="row"><span role="columnheader">{"Ticket ID"}</span><span role="columnheader">{"Category"}</span><span role="columnheader">{"Status"}</span><span role="columnheader">{"Filed on"}</span></div>{query.data.map((item, index) => <Link key={item.id} to={`/complaints/${item.id}`} className="complaint-table__row" style={{ "--row-delay": `${Math.min(index, 8) * 35}ms` } as CSSProperties} role="row" aria-label={fill("Open complaint {id}", { id: item.human_id })}><strong role="cell">{item.human_id}</strong><span className="complaint-category" role="cell"><i aria-hidden="true">{categoryLetter(item.category)}</i><span><b>{humanizeKey(item.category) ?? "Category pending"}</b><small>{item.priority === "high" ? "Priority" : item.priority ? "Normal priority" : "Municipal routing"}</small></span></span><span role="cell"><StatusChip status={item.status} /></span><time role="cell" dateTime={item.created_at}>{new Date(item.created_at).toLocaleDateString(bcp47, { day: "numeric", month: "short", year: "numeric" })}</time></Link>)}</div>}
  </div>;
}
